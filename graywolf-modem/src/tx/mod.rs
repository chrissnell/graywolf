...
/// frame at 1200 baud Bell 202 AFSK.
///
...
/// bracket the frame with the corresponding duration of HDLC flag bytes;
/// `txdelay_ms` is the *total* audio preamble and is what covers the
/// radio's transmitter attack time (no separate PTT-to-audio dead period).
///
/// `sample_rate` is the output sample rate in Hz — typically 48000. Zero
/// returns [`TxError::InvalidSampleRate`].
...
/// at 1200 baud. Each flag is 8 bits = 6.67 ms; we round up so the preamble
/// is never shorter than the caller asked for.
fn flags_for_ms(ms: u32) -> usize {
    ((ms as u64) * 150).div_ceil(1000) as usize
}
...
mod tests {
    ...
    #[test]
    fn flags_for_ms_rounds_up() {
        assert_eq!(flags_for_ms(0), 0);
        assert_eq!(flags_for_ms(1), 1); // 0.15 → 1
        assert_eq!(flags_for_ms(100), 15);
        assert_eq!(flags_for_ms(300), 45);
        assert_eq!(flags_for_ms(53), 8); // 7.95 → 8
    }
    ...
    fn build_samples_length_equals_encoded_bits_times_samples_per_bit_at_48k() {
        ...
        let preamble = flags_for_ms(300);
        let postamble = flags_for_ms(100);
        let bits = hdlc_encode::encode(&frame, preamble, postamble);
        let samples = build_samples(&frame, 300, 100, 48_000).unwrap();
    ...
    fn zero_sample_rate_returns_invalid_sample_rate_error() {
        let err = build_samples(&sample_beacon(), 300, 100, 0).unwrap_err();
