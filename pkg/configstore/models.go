...
// Bulletin represents an APRS bulletin (BLN0–BLN9) or announcement
// (BLNA–BLNZ). Inbound rows are upserted on (from_call, slot) so each
// station's per-slot content reflects the most recent received text.
// Outbound rows are driven by the bulletins scheduler, which re-sends
...
type Bulletin struct {
	ID             uint64         `gorm:"primaryKey;autoIncrement" json:"id"`
	Direction      string         `gorm:"size:3;not null;default:'in'" json:"direction"` // "in" | "out"
	Slot           string         `gorm:"size:5;not null" json:"slot"`                   // "BLN0".."BLNZ"
	FromCall       string         `gorm:"size:9;not null" json:"from_call"`
	Text           string         `gorm:"size:200;not null;default:''" json:"text"`
	Source         string         `gorm:"size:2" json:"source"` // "rf" | "is" (inbound)
	Channel        uint32         `gorm:"not null;default:0" json:"channel,omitempty"`
	RawTNC2        string         `gorm:"column:raw_tnc2;size:512" json:"raw_tnc2,omitempty"`
	IsAnnouncement bool           `gorm:"not null;default:false" json:"is_announcement"` // true for BLNA-Z
...
// FavoriteStation is one callsign the operator wants surfaced beyond the
// map: stationNewTransport.js notifies for a favorite whenever it's
// heard again after going unheard for the same operator-configurable
// staleness threshold used by the general "new station" notification
// (notificationPrefsState.stationNewThresholdSecs), but -- unlike the
// general path -- bypasses the Live Map's RF Only / Direct RX Only
// filter and is not excluded for being a digipeater, since a deliberate
// per-callsign favorite is assumed to always be wanted. No Enabled flag
// like BlockedCallsign's: removing a favorite IS the "I don't want this
// anymore" action, there's no meaningful "temporarily disabled" state.
// Server-side (not device-local) so the list is shared across every
// device pointed at this graywolf instance, matching BlockedCallsign.
type FavoriteStation struct {
	ID        uint32    `gorm:"primaryKey;autoIncrement" json:"id"`
	Callsign  string    `gorm:"size:9;not null;uniqueIndex" json:"callsign"` // 1-9 [A-Z0-9-], uppercase; optional -SSID
	Note      string    `gorm:"size:128" json:"note"`                        // optional free-text (e.g. "Dad's HT")
	CreatedAt time.Time `json:"created_at"`
}

// BeforeSave normalizes Callsign to uppercase and trims whitespace, same
// rationale as BlockedCallsign.BeforeSave.
func (f *FavoriteStation) BeforeSave(_ *gorm.DB) error {
	f.Callsign = strings.ToUpper(strings.TrimSpace(f.Callsign))
	return nil
}

// ExcludedStation is one callsign the operator never wants a new-station
// or favorite-station notification for -- the inverse of FavoriteStation,
// for a station the heuristics (digipeater/repeater/weather-station
// detection) don't catch but the operator recognizes as infrastructure
// (or simply doesn't want to hear about). stationNewTransport.js checks
// this before the favorite check, so an exclusion wins even over a
// favorite -- the more specific "no" takes precedence. Same wildcard
// base-callsign matching as FavoriteStation (a bare entry matches every
// SSID of that base call). Server-side, shared across every device
// pointed at this graywolf instance, same rationale as FavoriteStation.
type ExcludedStation struct {
	ID        uint32    `gorm:"primaryKey;autoIncrement" json:"id"`
	Callsign  string    `gorm:"size:9;not null;uniqueIndex" json:"callsign"` // 1-9 [A-Z0-9-], uppercase; optional -SSID
	Note      string    `gorm:"size:128" json:"note"`                        // optional free-text (e.g. "local digi, no icon")
	CreatedAt time.Time `json:"created_at"`
}

// BeforeSave normalizes Callsign to uppercase and trims whitespace, same
// rationale as BlockedCallsign.BeforeSave.
func (e *ExcludedStation) BeforeSave(_ *gorm.DB) error {
