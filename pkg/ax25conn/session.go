...
func (s *Session) cleanup() {
	...
	s.hb.stop()
	s.tStats.stop()
	// A terminal transition already emitted the DISCONNECTED state via
	// setState; only emit here when the loop exits some other way (ctx
	// cancel from Manager.Close while still in a live state) so the
	// bridge gets exactly one DISCONNECTED, not a duplicate.
	if !s.terminated {
