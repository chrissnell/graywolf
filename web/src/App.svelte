<script>
  ...
  // Every route is lazy-loaded: each one is its own chunk, fetched only when
  // the operator actually navigates there, with RouteLoading shown while it
  // fetches. (The map-heavy routes were split out first to fix a large
  // vendor-map bundle loading on every page; extending it to the rest of the
  // app was deferred until that pattern was proven safe.)
  const lazy = (loader) => wrap({ asyncComponent: loader, loadingComponent: RouteLoading });
  ...
  const baseRoutes = {
    ...
    '/preferences/beacons': lazy(() => import('./routes/CoTSettings.svelte')),
    '/preferences/maps': lazy(() => import('./routes/MapsSettings.svelte')),
    '/preferences/navigation': lazy(() => import('./routes/NavigationSettings.svelte')),
