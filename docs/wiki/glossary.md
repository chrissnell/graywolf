...
| `graywolf flare` CLI | The user-facing subcommand that collects a diagnostic flare from the local install, scrubs it, presents a review TUI, and submits to graywolf-flare-server. | [`../../graywolf/cmd/graywolf/flare.go`](../../graywolf/cmd/graywolf/flare.go) |
| Redact rule | One regex + replacement pair in `pkg/diagcollect/redact/rules.go`. Each rule has a positive and a negative test fixture; coverage is enforced by `rules_test.go`. | [`../../graywolf/pkg/diagcollect/redact/rules.go`](../../graywolf/pkg/diagcollect/redact/rules.go) |
| Portal token | The unguessable per-flare URL token returned by graywolf-flare-server. Saved to `~/.local/state/graywolf/flares/<flare-id>.json` (mode 0600) so `graywolf flare --resubmit FLARE-ID` can reuse it. | [`../../graywolf/pkg/diagcollect/submit/store.go`](../../graywolf/pkg/diagcollect/submit/store.go) |
| Resubmit | `graywolf flare --resubmit FLARE-ID` re-collects fresh data and posts to `/api/v1/submit/<id>/update`. Requires a saved portal token. | [`../../graywolf/pkg/diagcollect/submit/resubmit.go`](../../graywolf/pkg/diagcollect/submit/resubmit.go) |
