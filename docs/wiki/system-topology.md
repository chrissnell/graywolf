...
| `maps.nw5w.com/radar/*` | smoothed NEXRAD radar tiles: `radar/<ts>.pmtiles` per frame + `radar/latest.json` pointer, range-served by the origin Worker; browser polls `latest.json` and paints recolorable dBZ isoband fills | generator in `~/dev/graywolf-maps` (`radar-contour/`), client [`../../web/src/lib/map/layers/radar.js`](../../web/src/lib/map/layers/radar.js) | (no dedicated page yet) |
...
The radar **generator** (`radar-contour`, a Rust CronJob on `big-bulky-1` that
composites Level II super-res into one PMTiles archive per frame) and its Helm
chart live in `~/dev/graywolf-maps`, alongside the R2/Worker/deploy surface and
the origin Worker `/radar/*` route. See that repo's
`docs/wiki/build-pipelines.md` for the pipeline. **Only the MapLibre client
lives here**: it polls `radar/latest.json` and paints recolorable dBZ isoband
fills ([`../../web/src/lib/map/layers/radar.js`](../../web/src/lib/map/layers/radar.js),
[`radar-source.js`](../../web/src/lib/map/sources/radar-source.js), mounted in
[`LiveMapV2.svelte`](../../web/src/routes/LiveMapV2.svelte)).
