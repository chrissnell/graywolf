...
### 16b. KISS hardware-TNC types default to a TX-capable TNC link

*Why:* A `tcp-client`, `serial`, or `usb-serial` KISS interface connects to a device that IS the radio (a hardware TNC, LoRa modem, or radio with a built-in KISS TNC), so its only useful default is `Mode=tnc` + `AllowTxFromGovernor=true` -- otherwise it registers no TX backend and silently transmits nothing (beacons fail with `no backend registered for channel`, issue #128) **and** misroutes its RX into the TX governor so the operator sees no received packets (the recurring "serial KISS receives nothing" report). When `Mode` is **omitted from the request**, both the API boundary (`dto.KissRequest.ToModel`) and the store backstop (`normalizeKissInterface`) apply this default via the shared predicate `configstore.KissTypeDefaultsTnc` (true for `tcp-client`/`serial`/`usbserial`). Only `tcp` (server) keeps the historical `modem` default -- its peer is an APRS app; `bluetooth` is force-`tnc`'d at the wiring layer (and by the UI). The web UI mirrors this: selecting any of these types on a new row sets `Mode=tnc` + governor-TX via the shared `$effect` in `Kiss.svelte` (the UI always sends `mode` explicitly, so this -- not the backend default -- is the operative path for interactive creation). An *explicitly supplied* `Mode` is always honored verbatim. Note `POST`/`PUT /api/kiss` is full-resource replace (`Store.UpdateKissInterface` does `db.Save`): a `PUT` that omits `mode` re-applies the default exactly as create does -- it does NOT merge against the persisted row. The one hazardous case -- silently enabling TX on a channel that also has an audio input device (a modem backend), which would double-transmit -- cannot occur: `validateKissInterface` independently rejects `tnc`+`AllowTxFromGovernor` on a modem-backed channel before the row is written (so the rare legitimate "graywolf-as-soundmodem over a serial/PTY link" setup must pick `Mode=modem` explicitly). Migration 20 (`kiss_tcp_client_tx_default`) and Migration 26 (`kiss_serial_tnc_default`) repair pre-existing rows of their respective types stuck at the old `modem`/`false` default, both skipping any row whose channel has an audio input device.

Source: [`../../pkg/configstore/models.go`](../../pkg/configstore/models.go) (`KissTypeDefaultsTnc`),
[`../../pkg/webapi/dto/kiss.go`](../../pkg/webapi/dto/kiss.go),
[`../../pkg/configstore/store.go`](../../pkg/configstore/store.go) (`normalizeKissInterface`),
[`../../pkg/configstore/migrate.go`](../../pkg/configstore/migrate.go) (`migrateKissTcpClientTxDefault`, `migrateKissSerialTncDefault`),
[`../../web/src/routes/Kiss.svelte`](../../web/src/routes/Kiss.svelte) (TNC-defaults `$effect`).
