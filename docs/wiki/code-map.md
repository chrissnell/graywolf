...
| `aprs` (device identification) | `LookupTocall`/`LookupMicEDevice` resolve a packet's AX.25 destination (TOCALL) or Mic-E comment suffix to a `DeviceInfo{Vendor, Model, Class}` -- see the `stationcache` row below for how `deviceFor()` consumes this. The lookup tables (`tocallDB`, `miceDB`, `miceLegacyDB` in `deviceid_data.go`) are a **vendored, auto-generated snapshot** of [aprsorg/aprs-deviceid](https://github.com/aprsorg/aprs-deviceid)'s `tocalls.yaml` (CC BY-SA 2.0, Hessu OH7LZB) -- the same table aprs.fi itself uses, which is why aprs.fi's "Device:" line and graywolf's usually agree. There is no CI job or `go generate` wiring this to upstream; it only updates when someone notices a gap (e.g. a station showing a device on aprs.fi but not here) and reruns the regen tool. **Never hand-edit `deviceid_data.go` beyond a one-line emergency patch** -- regenerate it instead: `curl -o /tmp/tocalls.yaml https://raw.githubusercontent.com/aprsorg/aprs-deviceid/main/tocalls.yaml && go run scripts/gen-deviceid.go /tmp/tocalls.yaml > pkg/aprs/deviceid_data.go && gofmt -w pkg/aprs/deviceid_data.go`. `LookupTocall` re-sorts `tocallDB` by pattern specificity at `init()`, so source order in the generated file doesn't affect matching -- the generator just mirrors upstream's YAML order for readable diffs. (2026-09-11 resync also fixed a transcription bug: the Yaesu FTM-350 Mic-E suffix had been hand-entered as `_\` instead of the correct `_"`, so it never matched.) | `deviceid.go`, `deviceid_data.go`, `scripts/gen-deviceid.go` |
...
|---|---|---|
| `beacon` | Position/object/tracker/custom/igate beacon scheduler (min-heap), smart-beacon, encoder. Per-beacon `position_format` column ('compressed'\|'uncompressed'\|'mic_e', added in configstore migration 23) selects the wire encoding; uncompressed and Mic-E beacons honor `ambiguity` 0..4 via `aprs.ApplyLatLonAmbiguity` and the K/L/Z destination variants. Encoder entry points: `pkg/beacon/builder.go` switches on `Format`; `PositionInfo` (uncompressed, APRS101 ch 6) and `CompressedPositionInfo` (APRS101 ch 9) live in `pkg/beacon/encoder.go`; `MicEPositionInfo` + `MicEDestination` (APRS101 ch 10) live in `pkg/beacon/mice.go`. Mic-E swaps the AX.25 destination with the lat-derived destination at frame-build time in `scheduler.go`. The Mic-E status/message code (APRS101 ch 10 table 8) is operator-selectable per beacon (`configstore.Beacon.MicEMessageCode`, a string enum like `position_format` -- see [glossary.md](glossary.md)'s Mic-E entry for why it's not the raw wire int); `Beacons.svelte` gates the Status select on `position_format === 'mic_e'` and requires a confirm dialog before committing Priority/Emergency. | [`../handbook/beacons.html`](../handbook/beacons.html) |
...
| `stationcache` | Heard-station cache (memory + persistent) and APRS-extract helpers. Each `Station`/`CacheEntry` carries `StatusCode`/`StatusText` (Mic-E message code, APRS101 ch 10 table 8, or a `'>'` status report's free text) -- `-1` is the "no status" sentinel, since wire code `0` means Emergency and would otherwise collide with Go's int zero value (every construction path -- `buildStationEntry`, `buildObjectEntry`, `MemCache.Update`'s new-station branch, `historydb.LoadRecent`'s hydration -- sets it explicitly). `Station`/`CacheEntry` also carry `Device *aprs.DeviceInfo` (vendor/model/class), resolved by `deviceFor()` in `extract.go` in three steps when the AX.25 destination (TOCALL) is unrecognized: `aprs.LookupTocall` first, then `aprs.LookupMicEDevice` (the vendor-specific 2-char suffix code modern radios append to the Mic-E status text, e.g. `"_4"` -> Yaesu FTM-500D -- this is how aprs.fi tells an FTM-500D from a generic "Yaesu" apart), and only if that also misses, the generic single-byte Mic-E manufacturer family already decoded onto `pkt.MicE.Manufacturer` (e.g. `"Yaesu/Other"`) -- the same three-source resolution `webapi.enrichPacket` uses for the packet log, just keyed off the third-party-unwrapped packet so a gated station's own device shows rather than the gating IGate's. `MemCache.Update`'s `updateMetadata` only overwrites `Station.Device` when the incoming entry resolved one (mirrors the `Source` guard), so a later unrecognized-tocall packet can't blank a previously identified device. Not persisted to `historydb` -- same "ephemeral, reset until next packet" treatment as `StatusCode`. `GET /api/stations` (`pkg/webapi/stations.go`) exposes both `StatusCode`/`StatusText` and `Device` on `StationDTO` (`status_code` deliberately not `omitempty`, for the same zero-value reason; `device` is `omitempty` -- most tocalls resolve, but not all); the Live Map popup (`web/src/lib/map/popup.js`, `popup-helpers.js`'s `deviceText`) renders it as a "Device: Vendor: Model (class)" line -- this is graywolf's own inference from data it already has, not a call out to aprs.fi or any external service; `GET /api/stations/alerts` returns just the subset currently at `StatusCode == 0` (Emergency), regardless of map viewport, for `stationAlertsTransport.js`; `GET /api/stations/roster` returns every currently-heard station (objects/items excluded, digipeaters flagged `is_digipeater` via `isDigipeaterHeuristic` -- H-bit path membership OR symbol code `#` in either table (or overlaid-numbered) OR a "digi" substring in the comment -- weather stations flagged `is_weather_station` via `isWeatherStationHeuristic` -- `Station.Weather != nil` OR symbol code `_` in either table, and repeaters flagged `is_repeater` via `isRepeaterHeuristic` -- symbol code `r` OR a "repeater"/"rptr" comment OR a frequency+offset-sign/tone comment pattern, see [notifications.md](notifications.md)) as a compact `StationRosterDTO`, also regardless of map viewport, for `stationNewTransport.js` to diff against a persisted per-callsign staleness map; `GET/POST/DELETE /api/stations/favorites` (`configstore.FavoriteStation`, `pkg/webapi/stations_favorites.go`) and `GET/POST/DELETE /api/stations/exclusions` (`configstore.ExcludedStation`, `pkg/webapi/stations_exclusions.go`, structurally identical to favorites but inverted) are the separate server-side CRUD resources that same transport also checks, exclusions taking precedence (see [notifications.md](notifications.md)). | (no dedicated page) |
...
## Duplicate IS/RF echo collapsing (display-only)

No-ack senders (`pkg/messages/bots.go`'s `WellKnownBots`, e.g. WXBOT)
send messages with no `MessageID`, so `messages.Router`'s dedup
(invariant #27) never runs for them — the same packet heard on RF
direct and via the APRS-IS gate persists as two separate rows. Fixed
on the display side only; see [invariant 64](invariants.md).

| Concern | Where |
|---|---|
| Collapsing logic | `web/src/lib/duplicate-echo-core.js` — `collapseDuplicateEchoes(messages)`, pure function; merges adjacent inbound bubbles with matching `from_call` + `text` within a 10s window into one display bubble carrying `mergedIds` (every underlying row id) + `mergedSources` (unique source list). Tests in the sibling `.test.js`. |
| Wiring | `web/src/components/messages/MessageThread.svelte` — `displayBubbles` (collapsed list feeds clustering, day separators, and the `{#each}`); the dwell/read IntersectionObserver batch-marks every id in `mergedIds`, not just the primary. |
| Badge rendering | `web/src/components/messages/MessageBubble.svelte` — `sourceBadges` renders one `Badge` per unique source (falls back to the single raw `msg.source` for non-collapsed bubbles). |
...
| `src/routes/` | One Svelte route per page (Dashboard, LiveMapV2, Channels, Beacons, Digipeater, Igate, Kiss, Agw, Ptt, Gps, AudioDevices, Messages, **Bulletins**, Terminal + TerminalTranscripts, PositionLog, MapsSettings, Preferences (`/preferences`, "General" nav label), MessagesSettings, **NotificationsSettings** (`/preferences/notifications` — popup mode + per-type message/bulletin sounds + favorites/exclusions list management, see [notifications.md](notifications.md)), **NotificationsLog** (`/notifications-log`, sidebar between APRS Logs and System Logs — device-local capped history of every notification actually raised, see [notifications.md](notifications.md)), Login, About, Logs, Simulation) |
...
| Manual recenter controls | Two buttons stacked top-right under MapLibre's `NavigationControl` (compass/zoom), added 2026-07-31, both sharing one extent via `LiveMapV2.svelte`'s `fitDefaultRadius(lat, lon)` helper: `fitBounds()`s a box `mapState.defaultViewRadiusMiles` across (operator-configurable, default 15, "Default view radius (miles)" number input in the layer card next to Time range, step 5) centered on the given point. Top: "Reset to default view" (`resetToDefaultView`) calls it centered on `dataStore.myPosition`, falling back to `map-store.svelte.js`'s exported `WORLD_CENTER`/`WORLD_ZOOM` when no position is available yet. Bottom: "Center on my station" (`recenterOnMyStation`) also calls it centered on `dataStore.myPosition`, but errors via a toast instead of falling back when no position is known -- the only remaining behavioral difference between the two buttons now that both share the same extent (previously "Center on my station" used a fixed `MY_POSITION_ZOOM`, which could visibly disagree with the radius-based "Reset to default view"). The radius input's `onchange` (`onDefaultViewRadiusChange`) also applies live to wherever the operator is currently panned -- `mapRef.getCenter()`, not the station's own position -- so adjusting the setting doesn't require clicking a recenter button afterward to see the effect. Both buttons set `didAutoFit` so the one-shot my-position/fit-to-stations effects don't immediately fight a manual recenter on the next poll tick, and both call `clearFocus()` first to drop any stale `#/map?focus=` deep-link state (see [invariant 68](invariants.md)'s follow-up) so a manual recenter always wins over wherever a prior notification click left the camera. `MY_POSITION_ZOOM` (`map-store.svelte.js`) is still used by the one-shot auto-recenter-on-load effect, just no longer by these two buttons. |
| `src/lib/maps/` | Offline-maps client glue (downloads-store, state-bounds, state-list, state-picker) |
| `src/lib/settings/` | Reactive prefs stores (units, maps, messages-preferences, theme, ui-scale, `log-prefs` — device-local APRS Logs display toggles: auto-refresh/auto-scroll (GH #373), non-ASCII data (GH #376), and `newestFirst` which reverses the packet log so the most recent packet is on top (GH #519, applied in `PacketLogViewer.svelte` by reversing the already-projected entries with no re-fetch, suppressing Chonky's bottom auto-scroll while it's on since new packets then arrive at the top); `notification-prefs` — device-local toast/OS/both mode plus per-type message/bulletin master on-off switches, pure logic in `notification-prefs-core.js`; `notification-sound` — device-local per-type (message/bulletin) sound enable + preset/custom selection, pure logic in `notification-sound-core.js`, backed by `../soundPresets.js` + `../notificationSoundsDb.js` (IndexedDB); see [notifications.md](notifications.md)) |
