...
| NEXRAD radar overlay | `src/lib/map/layers/radar.js` (imperative MapLibre layer) consumes a descriptor from `src/lib/map/sources/radar-source.js`. The source module is the **backend seam**: `ACTIVE_RADAR_BACKEND` selects `raster` (IEM N0Q national composite) or `vector` (GRA-48 smoothed contour MVT). **Vector is the active backend**; flipping that one constant swaps backends — the layer module and `LiveMapV2.svelte` are backend-agnostic. The vector source is bounded to `minzoom 3`/`maxzoom 8` (the generated archive; keep in sync with the generator's `--min-zoom`/`--max-zoom`); the `.pbf` route is cycle-less, so `radar.js` refreshes frames by swapping in a `?v=<5-min bucket>` cache-bust template on each tick. Toggle + opacity persisted to `gw_radar_*` localStorage keys. Pure logic is unit-tested in `radar-source.test.js`; the cache-bust in `radar.test.js`. Tiles are served by the graywolf-maps origin Worker under `https://maps.nw5w.com/radar/*` (raster = edge-cached IEM pull-through; `.pbf` = GRA-48 contours from R2), riding the same `?t=` bearer token as the basemap. |
